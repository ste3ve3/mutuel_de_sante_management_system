...
//   import moment from 'moment';
  ...
  const UsersReport = () => {
    ...
    const { data, isError, isLoading } = useFetcher(
      `/person?date=${JSON.stringify(date)}`
    );
    ...
    const exportPDF = () => {
      ...
      doc.text(
        `UNDERPRIVILEGED USERS REPORT FOR ${
    ...
    const authorsTableData = {
        columns: [
          ...
          { name: "National Id", align: "center" },
          { name: "Resident Cell", align: "center" },
    ...
    <Card sx={{ py: 3 }}>
        <Container>
          ...
          >
            <Typography variant="h5">The Underprivileged Report</Typography>
          </Stack>
          ...
          <DataWidget
            ...
            isError={
              !isLoading && !users.length && isError ? isError : null
            }
            isEmpty={!isLoading && !users.length && !isError}
            customEmptyMessage={`There are no users available in this ${
              date.isYearly
                ? 'year, ' + date.year
